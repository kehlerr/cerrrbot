...
LOGGING_LEVEL = config(
    "CERRRBOT_LOGGING_LEVEL", default="DEBUG" if DEBUG else "INFO"
).upper()
...
DATA_DIRECTORY_ROOT = config(
    "DATA_DIR_PATH", default=os.path.join(os.getcwd(), "appdata")
)
assert os.path.isdir(
    DATA_DIRECTORY_ROOT
), f"DATA_DIR_PATH doesn't exists: {DATA_DIRECTORY_ROOT}"
...
REDIS_DEFAULT_CACHE_DB_IDX = config("CERRRBOT_REDIS_DEFAULT_CACHE_DB_IDX", default=2, cast=int)
REDIS_DEFAULT_CACHE_KEY_PREFIX_NOTIFICATION = config(
    "REDIS_DEFAULT_CACHE_KEY_PREFIX_NOTIFICATION", default="cerrrbot_cache"
)

REDIS_NOTIFICATIONS_DB_IDX = config("CERRRBOT_REDIS_NOTIFICATIONS_DB_IDX", default=3, cast=int)
REDIS_NOTIFICATIONS_CACHE_KEY_PREFIX = config(
    "REDIS_NOTIFICATIONS_CACHE_KEY_PREFIX", default="cerrrbot_notification"
)

DEFAULT_CHECK_FOR_NOTIFICATIONS_TIMEOUT = config(
    "DEFAULT_CHECK_FOR_NOTIFICATIONS_TIMEOUT", default=10, cast=int
)
...
DEFAULT_CHECK_FOR_NEW_MESSAGES_TIMEOUT = config(
    "DEFAULT_CHECK_FOR_NEW_MESSAGES_TIMEOUT", default=3, cast=int
)

DEFAULT_CHECK_FOR_DEPRECATED_MESSAGES_TIMEOUT = config(
    "DEFAULT_CHECK_FOR_DEPRECATED_MESSAGES_TIMEOUT", default=60, cast=int
)


PLUGINS_MODULE_NAME = config("PLUGINS_MODULE_NAME", default="plugins")


PLUGINS_DIR_PATH = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), PLUGINS_MODULE_NAME
)
assert os.path.isdir(
    PLUGINS_DIR_PATH
), f"PLUGINS_DIR_PATH doesn't exists: {PLUGINS_DIR_PATH}"
